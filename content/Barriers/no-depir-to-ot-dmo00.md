...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the page records an uncited, hedged note rather than a result, and `depir` is ambiguous across variants: unkeyed and public-key DEPIR are non-trivial single-server PIR, so [[DMO00 - Single Database Private Information Retrieval Implies Oblivious Transfer|DMO00]] applies and they imply OT, contradicting the wiki's own chain through CPIR. Only secret-key DEPIR plausibly evades DMO00, because the client rather than the server runs $\Setup$ and sees the whole database, and even there the evasion is not a theorem. See [[depir-to-cpir]] and [[cpir-to-ot-dmo00]].
