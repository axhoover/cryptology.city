...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — `depir` spans variants, and unkeyed and public-key DEPIR are non-trivial single-server PIR, so [[DMO00 - Single Database Private Information Retrieval Implies Oblivious Transfer|DMO00]] gives OT from them, contradicting the wiki's chain through CPIR. Only secret-key DEPIR plausibly evades DMO00, since the client, not the server, runs $\Setup$ on the whole database; even that is not a theorem. See [[depir-to-cpir]] and [[cpir-to-ot-dmo00]].
