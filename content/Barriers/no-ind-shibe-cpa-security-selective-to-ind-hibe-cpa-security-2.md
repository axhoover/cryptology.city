...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the page duplicates [[no-ind-shibe-cpa-security-selective-to-ind-hibe-cpa-security]] (same source sentence and endpoints, via the alias pair `selective-hibe-security`/`adaptive-hibe-security`), and nothing rules out constructing an adaptively secure HIBE from a selectively secure one: complexity leveraging does so at loss $|\Sigma^{\le d}|$, exponential in $d \log|\Sigma|$ rather than $d$, and no polynomial-loss impossibility is known. [[LW14 - Why Proving HIBE Systems Secure Is Difficult|LW14]] lower-bound the loss of simple black-box reductions from non-interactive assumptions to adaptive security for HIBE with checkable keys and ciphertexts, a different hyperedge.
