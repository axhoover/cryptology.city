...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the page duplicates [[no-ind-shibe-cpa-security-selective-to-ind-hibe-cpa-security]] (same source sentence, same endpoints through the alias pair `selective-hibe-security`/`adaptive-hibe-security`), and under the construction reading nothing rules out building an adaptively secure HIBE from a selectively secure one: complexity leveraging does it at loss $|\Sigma^{\le d}|$, exponential in $d \log|\Sigma|$ rather than in $d$, and no polynomial-loss impossibility is known. The nearest attributable result, [[LW14 - Why Proving HIBE Systems Secure Is Difficult|LW14]], lower-bounds the loss of simple black-box reductions from non-interactive assumptions to the adaptive security of HIBE schemes with checkable keys and ciphertexts, a different hyperedge. See [[no-ind-shibe-cpa-security-selective-to-ind-hibe-cpa-security]].
