...
- No citation.
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the source sentence records only that the LPN search-to-decision reduction does not immediately transfer to expanding matrices, an open question rather than a proved impossibility, and search-to-decision reductions for sparse $k$LIN over $\FF_2$ are known (Bogdanov–Rosen–Tan, CRYPTO 2025), so with `strength: unconditional` and a `contradiction` consequence the page states a theorem that does not exist. The orientation is correct, so the fix is deletion, with the open question (no search-to-decision reduction is known for expanding matrices over $\FF_p$) recorded on the assumption page. See [[noisy-k-lin-over-expanders#search-noisy-k-lin|Search noisy $k$-LIN]].
