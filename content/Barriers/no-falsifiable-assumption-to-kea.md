...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — no theorem rules out deriving KEA-like assumptions from falsifiable ones: [[GW11 - Separating Succinct Non-Interactive Arguments From All Falsifiable Assumptions|GW11]] rule out black-box reductions from falsifiable assumptions to the adaptive soundness of SNARGs (conclusion SNARK, not KEA), and Bitansky–Canetti–Paneth–Rosen (STOC 2014) refute extractable one-way functions with unbounded auxiliary input assuming indistinguishability obfuscation, rather than bar a reduction to them. A black-box reduction to KEA is not well-typed, since a KEA violation is not witnessed by an efficient adversary the reduction could run as an oracle. See [[no-falsifiable-assumption-to-snark-gro16]].
