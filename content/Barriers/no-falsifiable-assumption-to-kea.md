...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — no theorem bars deriving KEA-like assumptions from falsifiable ones: [[GW11 - Separating Succinct Non-Interactive Arguments From All Falsifiable Assumptions|GW11]] bar black-box reductions from falsifiable assumptions to SNARG adaptive soundness (conclusion SNARK, not KEA), and Bitansky–Canetti–Paneth–Rosen (STOC 2014) refute extractable one-way functions with unbounded auxiliary input assuming indistinguishability obfuscation, a refutation rather than a barrier. A black-box reduction to KEA is ill-typed: a KEA violation is not witnessed by an efficient adversary the reduction could run. See [[no-falsifiable-assumption-to-snark-gro16]].
