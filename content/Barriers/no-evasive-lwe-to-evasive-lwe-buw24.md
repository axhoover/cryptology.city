...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — hypothesis `private-coin-evasive-lwe` and conclusion `evasive-lwe` both resolve to the single anchor `learning-with-errors#evasive-lwe`, so the hyperedge is a self-loop, and [[BUW24 - Evasive LWE Assumptions Definitions Classes and Counterexamples|BUW24]] and [[AMYY25 - Evasive LWE Attacks, Variants & Obfustopia|AMYY25]] give counterexamples refuting several private-coin (and one circular public-coin) evasive-LWE variants — an `# Attacks` entry on the assumption page, not a reduction barrier. The migrated attribution also understates AMYY25, whose counterexamples reach the public-coin regime (the circular public-coin variant of HLL23 used for unbounded-depth ABE, and a public-coin formulation whose pre-condition error exceeds its post-condition error). See [[no-evasive-lwe-to-evasive-lwe-buw24-2]] (near-duplicate).
