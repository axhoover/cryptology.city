...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the hyperedge crosses functionality (KP- vs CP-ABE) and security notion (selective vs adaptive) at once, while the source sentence only observes that one proof strategy, the syntactic $\KeyGen$/$\Enc$ swap, does not carry a security proof across; no result rules out the composed reduction. The edges worth recording instead are that selective ABE security does not imply adaptive ABE security, by the folklore counterexample of the HIBE case, and the [[LW14 - Why Proving HIBE Systems Secure Is Difficult|LW14]] barrier against simple black-box reductions to non-interactive assumptions for ABE with checkable keys and ciphertexts. See [[no-ind-shibe-cpa-security-selective-to-ind-hibe-cpa-security]].
