...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the edge is inverted: $\classQCMA \subseteq \classQMA$ holds (a classical witness is a quantum state) and is already recorded, while the cited papers separate $\classQMA \not\subseteq \classQCMA$ relative to a quantum unitary oracle ([[AK07 - Quantum versus Classical Proofs and Advice|AK07]]) and a classical oracle ([[BHNZ25 - Separating QMA from QCMA with a Classical Oracle|BHNZ25]], reproved by [[BHV26 - Separating Quantum and Classical Advice with Good Codes|BHV26]]), so the correct edge has hypothesis QMA and conclusion QCMA, class `relativizing`. The page is also a chronology of six hyperedges, not one claim: BFM23, NN23 and BK24 are restricted (in-place quantum oracle; distributional; bounded adaptivity), and AK07 and BHV26 also separate BQP/qpoly from BQP/poly. See [[qcma-to-qma]].
