...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the edge is inverted: $\classQCMA \subseteq \classQMA$ holds (a classical witness is a quantum state) and is already recorded as an inclusion, while the cited papers separate the opposite inclusion, $\classQMA \not\subseteq \classQCMA$, relative to a quantum unitary oracle ([[AK07 - Quantum versus Classical Proofs and Advice|AK07]]) and to a standard classical oracle ([[BHNZ25 - Separating QMA from QCMA with a Classical Oracle|BHNZ25]], reproved by [[BHV26 - Separating Quantum and Classical Advice with Good Codes|BHV26]]), so the correct edge has hypothesis QMA, conclusion QCMA and class `relativizing`. The page is also a chronology rather than a claim: three of the six results are restricted (BFM23 in-place quantum oracle; NN23 distributional; BK24 bounded adaptivity) and two carry a second, BQP/qpoly versus BQP/poly separation, and each is its own hyperedge. See [[qcma-to-qma]].
