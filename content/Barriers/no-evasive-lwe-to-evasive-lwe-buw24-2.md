...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — both endpoints resolve to the single anchor `learning-with-errors#evasive-lwe`, so the hyperedge is a self-loop, and [[BUW24 - Evasive LWE Assumptions Definitions Classes and Counterexamples|BUW24]] and [[AMYY25 - Evasive LWE Attacks, Variants & Obfustopia|AMYY25]] give counterexamples refuting the private-coin and circular formulations of evasive LWE, not a reduction barrier. The supportable edge, if any, is the reverse (restricted public-coin evasive LWE does not imply the private-coin variant) and must name the restricted formulation, since AMYY25 also attack a public-coin formulation whose pre-condition error is larger than its post-condition error. See [[no-evasive-lwe-to-evasive-lwe-buw24]] (near-duplicate).
