...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — both endpoints resolve to `learning-with-errors#evasive-lwe`, so the hyperedge is a self-loop, and [[BUW24 - Evasive LWE Assumptions Definitions Classes and Counterexamples|BUW24]] and [[AMYY25 - Evasive LWE Attacks, Variants & Obfustopia|AMYY25]] refute the private-coin and circular formulations by counterexample rather than bar a reduction. The supportable edge, if any, is the reverse — restricted public-coin evasive LWE, conjecturally hard, does not imply the refuted private-coin variant — and must name the restriction, since AMYY25 also attack a public-coin formulation with pre-condition error larger than post-condition error. See [[no-evasive-lwe-to-evasive-lwe-buw24]] (near-duplicate).
