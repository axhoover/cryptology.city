...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the edge is inverted: $\mathrm{MIP} \subseteq \mathrm{MIP}^*$ holds (Ito–Vidick's $\mathrm{NEXP} \subseteq \mathrm{MIP}^*$; also $\mathrm{MIP}^* = \mathrm{RE} \supseteq \mathrm{NEXP} = \mathrm{MIP}$), so barring it is false; the separation the migrated prose asserts is the `free` barrier from MIP\* to MIP. Its gloss "exponentially (in fact, incomparably) increases prover power" is also wrong: RE versus NEXP is an undecidable-versus-decidable gap.
