...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the edge is inverted: $\mathrm{MIP} \subseteq \mathrm{MIP}^*$ holds (completeness is immediate, soundness against entangled provers is Ito–Vidick's $\mathrm{NEXP} \subseteq \mathrm{MIP}^*$, and it also follows from $\mathrm{MIP}^* = \mathrm{RE} \supseteq \mathrm{NEXP} = \mathrm{MIP}$), so a barrier ruling it out is false. The strict separation the migrated prose asserts is a barrier from MIP\* to MIP (no inclusion $\mathrm{MIP}^* \subseteq \mathrm{MIP}$) of class `free`, and its gloss "exponentially (in fact, incomparably) increases prover power" is also wrong, since RE versus NEXP is an undecidable-versus-decidable gap, not an exponential one.
