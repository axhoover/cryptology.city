...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the conclusion is a construction that exists: Merkle's puzzles is a random-oracle key-agreement protocol with a quadratic query gap ([[Mer78 - Secure Communications Over Insecure Channels|Mer78]]), so a barrier against ROM ⇒ Merkle puzzles is false. [[BM09 - Merkle Puzzles Are Optimal An O(n2)-Query Attack on Any Key Exchange from a Random Oracle|BM09]] bound the different hyperedge ROM ⇒ KE — every random-oracle key agreement whose honest parties make $n$ queries is broken by an eavesdropper making $O(n^2)$ queries, so none achieves a better-than-quadratic query gap — which belongs on a barrier with conclusion `ke`, class `free` and model `rom`, reconciled with the BM09 strand now bundled into the page below. See [[no-rom-to-ke-hmo-19]].
