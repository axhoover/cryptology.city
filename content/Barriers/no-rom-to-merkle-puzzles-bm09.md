...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — Merkle's puzzles is a random-oracle key-agreement protocol with a quadratic query gap ([[Mer78 - Secure Communications Over Insecure Channels|Mer78]]), so barring ROM ⇒ Merkle puzzles is false. [[BM09 - Merkle Puzzles Are Optimal An O(n2)-Query Attack on Any Key Exchange from a Random Oracle|BM09]] bound the different hyperedge ROM ⇒ KE (every random-oracle key agreement with $n$ honest queries falls to an $O(n^2)$-query eavesdropper), which belongs on a barrier with conclusion `ke`, class `free`, model `rom`. See [[no-rom-to-ke-hmo-19]], which now carries that strand.
