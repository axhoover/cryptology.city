...
## Syntax
Throughout, we use the following notation. All oblivious RAM schemes are defined with respect to a key space $\mathsf{KSp}$, state space $\mathsf{StSp}$, virtual array size $N_1$ and blocks $\calB_1$, and physical array size $N_2$ and blocks $\calB_2$. We additionally assume $\bot \in \mathsf{StSp}$ and $\bot \notin \calB_1$. Furthermore, we define the sets:
