...
- $\PKE$ secure against polynomial-size adversaries from the planted $n^{\log^\alpha n}$-clique conjecture and noisy $k$-LIN over $(\gamma, \Omega(\log n), 2^{(\log n)^\alpha})$-expanders over $\FF_p$ — [[GHJS25 - Public-Key Encryption from Planted Clique and Noisy k-LIN Over Expanders|GHJS25]]
