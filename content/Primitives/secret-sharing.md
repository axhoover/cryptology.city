...
- The Razborov–Gál rank measure lower-bounds the normalized size of a multi-target monotone span program, so monotone-span-program rank lower bounds survive the share-size amortization available to multilinear schemes — [[Kha26 - Rank Measures and Exponential Lower Bounds for Multilinear Secret Sharing|Kha26]]
- An explicit family of access structures forces average and maximum information ratio $2^{\Omega(n)}$ on every perfect multilinear scheme over every finite field, placing the worst-case multilinear information ratio at $2^{\Theta(n)}$ — [[Kha26 - Rank Measures and Exponential Lower Bounds for Multilinear Secret Sharing|Kha26]]
