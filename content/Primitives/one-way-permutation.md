...
A _one-way permutation_ is a family of efficiently computable permutations $\{\pi_{\secpar} : \calD \to \mathcal{D}\}_{\secpar \in \mathbb{N}}$ and a distribution $X$ over $\mathcal{D}$, such that there is some negligible function $\nu$, where, for every $\secpar$ and efficient algorithm $\calA$: $$\Pr_{x\sim X}[\pi_{\secpar}(x') = \pi_{\secpar}(x) : x' \gets \mathcal{A}(1^{\secpar}, \pi_{\secpar}(x))] \le \nu(\secpar).$$
