...
The class of counting problems associated with NP decision problems. Formally, a function $f : \bits^* \to \NN$ is in $\classsharpP$ if there exists a nondeterministic polynomial-time Turing machine $M$ such that $f(x)$ equals the number of accepting computation paths of $M$ on input $x$.
