...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the hypothesis set omits sub-exponential SXDH on asymmetric prime-order bilinear groups, which [[JLS21 - Indistinguishability obfuscation from well-founded assumptions|JLS21]] also assume; without a bilinear-group assumption, LWE, LPN, and a PRG are not a known route to iO. JLS21's PRG is a Boolean PRG in $\mathrm{NC}^0$ (not $\mathrm{NC}^1$) with stretch $n^{1+\tau}$, and its LPN is over $\ZZ_p$ with inverse-polynomial error rate. See [[ddh-and-lpn-and-lwe-and-nc1-prg-to-io-jls21]].
