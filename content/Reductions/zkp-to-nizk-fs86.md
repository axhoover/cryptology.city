...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — without setup, only languages in $\classBPP$ have two-message (hence one-message) auxiliary-input zero-knowledge proofs or arguments (Goldreich–Oren, J. Cryptology 1994), so `model: standard` gives no NIZK for any $L \notin \classBPP$. With a CRS no generic compiler is known: FS86 needs the random-oracle model and a public-coin protocol, BFM88 a number-theoretic assumption. See [[rom-and-zkp-to-nizk-fs86]] and [[tdp-to-nizk-bfm88]].
