...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — every language in $\classBPP$ has a trivial unconditional NIZK, while commitments imply one-way functions, so the unqualified implication would build OWFs from nothing. The correct edge is computational NIZK for a hard-on-average language ⇒ OWF, in the public-parameter model (Pass–shelat, CRYPTO 2005); commitments follow only through a separate chain.
