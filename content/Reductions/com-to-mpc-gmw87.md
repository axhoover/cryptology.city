...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — GMW87 build the semi-honest protocol from oblivious transfer; commitments (via zero-knowledge proofs for NP) enter only in the compiler to malicious security. COM ⇒ semi-honest MPC is not a known theorem: commitments follow from OWFs while semi-honest 2PC for all functionalities implies OT, so it would place OT in Minicrypt. See [[ot-to-mpc-kil88]].
