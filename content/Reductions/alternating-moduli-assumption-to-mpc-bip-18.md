...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — honest-majority MPC exists unconditionally [[BGW88 - Completeness theorems for non-cryptographic fault-tolerant distributed computation|BGW88]], so that reading is vacuous, while dishonest-majority MPC requires OT and no construction of OT from AM hardness is known (the assumption yields a weak PRF). The migrated bullet records an efficiency property of the concrete [[BIP+18 - Exploring Crypto Dark Matter New Simple PRF Candidates and Their Applications|BIP+18]] candidates (low-round, low-communication distributed evaluation in the honest-majority and preprocessing settings), which belongs as an application note on the assumption page rather than an existence edge to MPC. See [[alternating-moduli]].
