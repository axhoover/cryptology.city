...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — no theorem takes a NIZK together with a polynomial commitment scheme and an arithmetization to a SNARK, and the construction behind the migrated sentence (arithmetize to a polynomial IOP, compile with an extractable PCS, apply Fiat–Shamir) uses no NIZK. The `— standard` label also misuses the folklore exception, since the result is attributable. See [[arithmetization-and-pcs-to-snark-kzg10]].
