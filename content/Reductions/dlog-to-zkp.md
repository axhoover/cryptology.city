...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the migrated passages assert no implication: the Schnorr protocol is a sigma protocol for the DLOG relation, special sound and perfectly HVZK unconditionally, so DLOG hardness is not a hypothesis for building it. Read as "ZK proofs for NP exist if DLOG is hard", the claim is true but composite (DLOG ⇒ OWF ⇒ PRG ⇒ commitment ⇒ ZK for NP), which the repo splits one page per link; only the DLOG ⇒ OWF link is missing. See [[owf-to-prg-hill99]], [[prg-to-com-naor91]], [[hash-function-to-zkp-gmw91]].
