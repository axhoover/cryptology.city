...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — the Fiat–Shamir transform preserves the prover's communication, so an arbitrary interactive protocol plus the ROM cannot yield a _succinct_ argument, and without round-by-round (state-restoration) soundness it does not preserve soundness for polynomially many rounds even in the ROM. The correctly hypothesised edges are PCP + ROM ⇒ SNARG (Micali, _Computationally Sound Proofs_, SIAM J. Comput. 2000) and public-coin IOP + ROM ⇒ SNARK ([[BCS16 - Interactive Oracle Proofs|BCS16]]). See [[ip-and-rom-to-snark-fs86]], which records the same claim.
