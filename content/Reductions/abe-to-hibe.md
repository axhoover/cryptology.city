...
- Sourcing pass (2026-09), **not fixed**: claim judged incorrect as stated — HIBE syntax includes $\Delegate$, which generic ABE lacks, so a black-box wrapper (key policy a conjunction over the prefix positions) captures only HIBE's prefix-decryption relation. The supportable edge is KP-ABE with key delegation ⇒ HIBE, where delegation is a property of the [[GPSW06 - Attribute-Based Encryption for Fine-Grained Access Control of Encrypted Data|GPSW06]] construction, not of the ABE primitive; no construction of HIBE from generic ABE is known.
